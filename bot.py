...
class Bot:
    ...
    async def _play_game(self):
        ...
        while is_started and not is_finished:
            ...
            while current_game_progress['whose_turn'] != self._player['color']:
                current_game_progress = await self._get_game()
                await asyncio.sleep(0.1)

            # storing last moves of the opponent
            if current_game_progress['last_move']['player'] != self._player['color']:
                for move in current_game_progress['last_move']['last_moves']:
